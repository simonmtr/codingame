...
int main()
{
    ...
    while (1)
    {
        ...
        cerr << "eggcells index sorted:" << endl;
        for (int i = 0; i < egg_cells.size(); i++)
        {
            cerr << "index of list: " << i << " - cell: " << egg_cells.at(i).index << " - dth: " << egg_cells.at(i).distance_to_home_base << endl;
        }
            ...
            if (find(egg_indexes.begin(), egg_indexes.end(), i) != egg_indexes.end())
            {
                ...
                for (int j = 0; j < egg_cells.size(); j++)
                {
                    if (egg_cells.at(j).index == i)
                    {
                        // if (resources == 0)
                        // {
                        //     egg_cells.erase(egg_cells.begin() + j);
                        // }
                        // else
                        // {
            ...
            if (find(crystal_indexes.begin(), crystal_indexes.end(), i) != crystal_indexes.end())
            {
                ...
                for (int j = 0; j < crystal_cells.size(); j++)
                {
                    if (crystal_cells.at(j).index == i)
                    {
                        // if (resources == 0)
                        // {
                        //     crystal_cells.erase(crystal_cells.begin() + j);
                        // }
                        // else
                        // {
        ...
        cerr << "ANTS: " << enemy_ants_much_higher_than_mine << " - " << opp_ants_total << " - " << my_ants_total << endl;
        if (egg_cells.size() != 0 && enemy_ants_much_higher_than_mine)
        {
            sample_cell = egg_cells.back();
        }
        else
        {
            cerr << "EGGCELLS EMPTY" << endl;
            sample_cell = crystal_cells.back();
        }

        // will lead to error for one turn when 2 cells get to 0 at same turn
        ...
        string printstring = "";
        int max_ants_per_field = my_ants_total / sample_cell.distance_to_home_base;
        // for (int i = 0; i < sample_cell.route_to_home_base.size(); i++)
        // {
        //     // printstring = printstring + "BEACON " + to_string(sample_cell.route_to_home_base.at(i)) + " " + to_string(max_ants_per_field) + ";";
        //     if (i == 0)
        //     {
        //         printstring = printstring + "BEACON " + to_string(sample_cell.route_to_home_base.at(i)) + " " + to_string(1) + ";";
        //     }
        //     else
        //     {
        //         printstring = printstring + "BEACON " + to_string(sample_cell.route_to_home_base.at(i)) + " " + to_string(sample_cell.route_to_home_base.size() - i) + ";";
        //     }
        // }
