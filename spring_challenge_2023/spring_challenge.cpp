...
struct Cell
{
    ...
    Cell(int index, int type, int initial_resources, int distance_to_home_base)
    {
        this->index = index;
...
int main()
{
    ...
    vector<int> dist = DijkstraSP(adjList, my_base);
    for (int i = 0; i < egg_cells.size(); i++)
    {
        egg_cells.at(i).distance_to_home_base = dist[egg_cells.at(i).index];
    }
    for (int i = 0; i < crystal_cells.size(); i++)
    {
        crystal_cells.at(i).distance_to_home_base = dist[crystal_cells.at(i).index];
    }
    ...
    // cerr << "distance from base to " << test_cell.index << "with dtb " << test_cell.distance_to_home_base << " is " << dist[test_cell.index] << endl;
    // PrintShortestPath(dist, my_base);
